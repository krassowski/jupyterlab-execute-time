...
export async function acceptDialog(page: Page, title: string) {
  ...
  const button = await dialog.waitForSelector('.jp-Dialog-button.jp-mod-accept');
  await button.click();
 ...
 */
export async function maskedScreenshot(widget: ElementHandle<HTMLElement | SVGElement>) {
